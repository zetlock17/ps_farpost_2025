...
class OutagePreprocessor:
    ...
    def extract_features(self, df, weather_df=None):
        ...
        features["is_weekend"] = (df["start_date"].dt.dayofweek >= 5).astype(
            int
        )
        ...
        if weather_df is not None:
            ...
            weather_df["date_only"] = pd.to_datetime(
                weather_df["date"]
            ).dt.date.astype(str)
            merged_df = pd.merge(df, weather_df, how="left", on="date_only")

            features["temp_max"] = merged_df["temp_max"].fillna(
                merged_df["temp_max"].mean()
            )
            features["temp_min"] = merged_df["temp_min"].fillna(
                merged_df["temp_min"].mean()
            )

            if "weather_type" in merged_df.columns:
                merged_df["weather_type"] = merged_df["weather_type"].fillna(
                    "unknown"
                )
                if "weather_type" not in self.label_encoders:
                    self.label_encoders["weather_type"] = LabelEncoder()
                    features["weather_type_encoded"] = self.label_encoders[
                        "weather_type"
                    ].fit_transform(merged_df["weather_type"])
                else:
                    new_types = set(merged_df["weather_type"]) - set(
                        self.label_encoders["weather_type"].classes_
                    )
                    if new_types:
                        self.label_encoders["weather_type"].classes_ = (
                            np.concatenate(
                    ...
                    features["weather_type_encoded"] = self.label_encoders[
                        "weather_type"
                    ].transform(merged_df["weather_type"])

        # Кодирование типа отключения
        if "type" not in self.label_encoders:
            self.label_encoders["type"] = LabelEncoder()
            features["type_encoded"] = self.label_encoders[
                "type"
            ].fit_transform(df["type"])
        else:
            features["type_encoded"] = self.label_encoders["type"].transform(
                df["type"]
            )
        ...
        if "address" in df.columns:
            ...
            if "street" not in self.label_encoders:
                ...
                features["street_encoded"] = self.label_encoders[
                    "street"
                ].fit_transform(df["street"])
            else:
                new_streets = set(df["street"]) - set(
                    self.label_encoders["street"].classes_
                )
                if new_streets:
                    self.label_encoders["street"].classes_ = np.concatenate(
                        [
                            self.label_encoders["street"].classes_,
                            list(new_streets),
                        ]
        ...
        if "description" in df.columns:
            ...
            features["has_emergency_words"] = (
                df["description"]
                .str.contains("авария|срочн|экстренн", case=False, na=False)
                .astype(int)
            )
            features["has_planned_words"] = (
                df["description"]
                .str.contains("планов|ремонт|профилакт", case=False, na=False)
                .astype(int)
            )
...
def train_model(model, train_loader, val_loader, epochs, lr, device):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 10 == 0:
            print(
                f"Эпоха [{epoch+1}/{epochs}], Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}"
            )
...
def main():
    ...
    scaled_features, labels = preprocessor.fit_transform(
        df, weather_df=weather_df
    )
